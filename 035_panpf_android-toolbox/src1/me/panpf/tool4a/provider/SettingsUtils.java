...
public class SettingsUtils {
     ...
     *
     * @param context     上下文
     * @param mediaVolume 媒体音量，取值范围为0-15
     */
    public static boolean setMediaVolume(Context context, int mediaVolume) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            AudioManager audioManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
            ...
            if (mediaVolume < 0) {
                mediaVolume = 0;
            } else if (mediaVolume > 15) {
                mediaVolume = mediaVolume % 15;
                if (mediaVolume == 0) {
                    mediaVolume = 15;
                }
            }
    ...
    public static int getRingVolume(Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            return Settings.System.getInt(context.getContentResolver(), "volume_ring", 0);
        }
    }
     ...
     *
     * @param context    上下文
     * @param ringVolume 铃声音量，取值范围为 0-7
     */
    public static boolean setRingVolume(Context context, int ringVolume) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            AudioManager audioManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
            if (audioManager != null) {
